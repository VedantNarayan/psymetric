-- Create the atomic sync_scenarios_snapshot function to run all updates, deletes, and version control commits in a single transaction.
    ...
    IF v_scenario ? 'questions' AND jsonb_typeof(v_scenario->'questions') = 'array' THEN
      FOR v_question IN SELECT * FROM jsonb_array_elements(v_scenario->'questions') LOOP
        ...
        INSERT INTO public.questions (
          id, scenario_id, sequence_order, question_text, show_at_seconds
        ) VALUES (
          v_question_id,
          v_scenario_id,
